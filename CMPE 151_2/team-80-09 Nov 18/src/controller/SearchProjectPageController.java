...
public class SearchProjectPageController implements Initializable {
    ...
    @FXML private Label	isUpdatedOrDeleted;
	...
	public void initialize(URL location, ResourceBundle resources) {
		UpdateTable_Project();
		search_Project();
		isUpdatedOrDeleted.setText("");
	}

	ObservableList<NewProjectData> listP_search;

	/**
	 * Update the table to display existing projects.
	 */
	...
	public void search_Project() {
		...
		// Create an instance of a SortedList and initialize with data of NewProjectData objects
	...
	/**
	 * Updates the data of a project in the database based on the values entered from the user. 
	 * It retrieves the updated values from the text fields and constructs an SQL update query with the database. 
	 * Then a message "Successfully Updated!" is displayed and the project table is updated.
	...
	public void Edit_Button() {
		try {
			...
			pst_Edit.execute();
			if(update1.length()>0) {
				isUpdatedOrDeleted.setText("Successfully Updated!");
			}
			UpdateTable_Project();
			search_Project();
			...
			isUpdatedOrDeleted.setText(e.getMessage());
		}
	}
	
	/**
	 * Deletes the data of a project in the database, including all tickets and comments associated with the project.
	 * Then a message "Data Deleted!" is displayed and the project table is updated.
	 * Otherwise, an error message is displayed.
	 */
	public void Delete_Button() {
		try {
			...
			pst_Delete.execute();
			if(update1.length()>0) {
				isUpdatedOrDeleted.setText("Data Deleted!");
			}
			UpdateTable_Project();
			search_Project();
			edit_project_ID.setText(null);
