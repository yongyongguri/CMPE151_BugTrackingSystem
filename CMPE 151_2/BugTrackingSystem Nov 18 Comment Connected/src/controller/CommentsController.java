...
public class CommentsController implements Initializable {
	...
	public void addComment_saveButton() {
		...
		try {
			...
			pst_Comment.setString(3, add_BugID_column.getCellObservableValue(Bug_tableView.getItems().get(0)).getValue());
			System.out.println(add_BugID_column.getCellObservableValue(Bug_tableView.getItems().get(0)).getValue().toString());
			pst_Comment.execute();
			pst_Comment.close();
	...
	public void UpdateTable_Bug() {
		...
		Bug_tableView.setItems(listB_LastValue);
	}
}
